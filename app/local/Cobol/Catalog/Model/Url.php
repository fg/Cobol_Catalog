...
/**
 * Class Cobol_Catalog_Model_Url
 */
class Cobol_Catalog_Model_Url extends Mage_Catalog_Model_Url {
    ...
    public function getProductRequestPath($product, $category)
    {
        ...
        if (isset($this->_rewrites[$idPath])) {
            ...
            if ($product->getUrlKey() == '' && !empty($requestPath)
                ...
                if (preg_match('#^-([0-9]+)$#i', $existingRequestPath)) {
                    echo $this->_rewrites[$idPath]->getRequestPath();
                }
            }
        ...
        $validatedPath = $this->getResource()->checkRequestPaths(
            array(Mage::getStoreConfig('catalog/seo_extra/product_url_prefix').$requestPath.'-p'.$product->getId().$suffix, $requestPath.'-'.$product->getId().$suffix),
            $storeId
        );
